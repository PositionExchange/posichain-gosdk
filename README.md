...
This is a go layer on top of the Posichain RPC, included is a CLI tool that you can build with a
simple invocation of `make`.

See https://docs.posichain.org for detailed
documentation on how to use the `psc` CLI tools
...
once built, add `psc` to your path and add to your `.bashrc`

```
. <(psc completion)
```

invoke the following command to see the most command usages of `psc`
...
Example:
```
psc --node="https://s1.t.posichain.org/" transfer --file ./batchTransactions.json
```
...
The go-sdk code respects `PSC_RPC_DEBUG PSC_TX_DEBUG` as debugging
based environment variables.

```bash
PSC_RPC_DEBUG=true PSC_TX_DEBUG=true ./psc blockchain protocol-version
```
